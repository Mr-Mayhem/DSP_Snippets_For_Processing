/*
EdgeDetection_1d.pde, a demo of edge detection in one dimension
...
void draw() {
  ...
  //    } else { 
  //      edges[outerPtr-1] = 0; // black
  //    } 
  //  } else if(output[outerPtr+1]*output[outerPtr-1] < 0){
  //    if(Math.abs(output[outerPtr+1])+Math.abs(output[outerPtr-1]) > edgeThresh){ 
  //      edges[outerPtr-1] = 255; //white
  //      } else { 
  //        edges[outerPtr-1] = 0; // black
...
//int [] generateZeroCrossings(float [] data, int imgWidth, boolean limiter, float limit){ 
   
//  int[] TempDataDest = new int[imgWidth];
//  float thresh; //Set the threshold value 
...
//      if(Math.abs(data[i-1]) + Math.abs(data[i+1]) > thresh){ 
//        TempDataDest[i-1] = 255; 
//      } else { 
//        TempDataDest[i-1] = 0; 
//      } 
